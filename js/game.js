...
const OAK_NAME = 'THE WISE OAK TREE';

function say(speaker, text, mood, cls, choices) {
  /* He is rooted. He cannot follow you, so he cannot speak anywhere but under
     himself — not on a timer, not when a season turns, not when a new subject
     opens while you are two places away. Anything he tries to say from a
     distance is kept until you are standing under him again. This is the one
     choke point, so no caller can get it wrong. */
  if (speaker === OAK_NAME && G.scene === 'game' && !G.dead && !G.cine && !atOak()) {
    tellHimLater(text, mood);
    return;
  }
...
function panelScrolls() { return panelMaxScroll() > 0; }

/* dragging the paper itself, which is the only way to scroll on a phone */
const PDRAG = { on: false, y0: 0, s0: 0, moved: 0, hit: null };

function panelScrollBy(d) {
  PANEL.target = Math.max(0, Math.min(panelMaxScroll(), PANEL.target + d));
  if (!save.learned.scroll) { save.learned.scroll = 1; persist(); }
}

/* the two arrow buttons at the right-hand edge */
function panelArrowBox(dir) {
  if (!panelScrolls() || PANEL.ink <= 0.5) return null;
  const b = panelBox();
  const x = b.x + b.w - 12, w = 12, h = 13;
  return { x, w, h, y: dir < 0 ? b.y + 2 : b.y + b.h - h - 2 };
}

function panelArrowAt(fx, fy) {
  for (const d of [-1, 1]) {
    const a = panelArrowBox(d);
    if (a && fx >= a.x && fx <= a.x + a.w && fy >= a.y && fy <= a.y + a.h) return d;
  }
  return 0;
}

function drawPanelArrows(c) {
  for (const d of [-1, 1]) {
    const a = panelArrowBox(d);
    if (!a) return;
    const spent = d < 0 ? PANEL.scroll < 1 : PANEL.scroll > panelMaxScroll() - 1;
    const hot = PANEL.arrowHover === d && !spent;
    SPR.px(c, a.x, a.y, a.w, a.h, PAP.deep);
    SPR.px(c, a.x + 1, a.y + 1, a.w - 2, a.h - 2, hot ? PAP.ink3 : PAP.dim);
    const col = spent ? PAP.deep : (hot ? PAP.lit : PAP.ink);
    const cy = a.y + (d < 0 ? 5 : 8);
    for (let i = 0; i < 4; i++) SPR.px(c, a.x + 2 + i, cy + (d < 0 ? i : -i), 8 - i * 2, 1, col);
  }
}
...
function drawPanelScroll(c) {
  ...
  // the bar, with room for an arrow button at each end, so it is obvious
  // that there is more paper below
  if (PANEL.contentH > PANEL.viewH && PANEL.ink > 0.5) {
    const trackY = y + 17, trackH = h - 34;
    const th = Math.max(8, trackH * PANEL.viewH / PANEL.contentH);
    const tp = (PANEL.scroll / (PANEL.contentH - PANEL.viewH)) * (trackH - th);
    SPR.px(c, x + PANEL.w - 11, trackY, 10, trackH, PAP.dim);
    SPR.px(c, x + PANEL.w - 11, trackY, 10, 1, PAP.deep);
    SPR.px(c, x + PANEL.w - 10, trackY + tp, 8, th, PAP.ink3);
    SPR.px(c, x + PANEL.w - 10, trackY + tp, 8, 1, PAP.ink2);
    drawPanelArrows(c);
    // and a hint at the very bottom of the paper the first few times
    if (PANEL.scroll < 2 && !save.learned.scroll) {
      const lab = 'MORE BELOW';
      const lw = F.textWidth(lab, 1);
      const bob = Math.round(Math.sin(G.t * 4) * 1);
      SPR.px(c, x + PANEL.w / 2 - lw / 2 - 4, y + h - 12 + bob, lw + 8, 10, PAP.ink3);
      F.drawTextCentered(c, x + PANEL.w / 2, y + h - 10 + bob, lab, PAP.lit, 1);
    }
  }
}
...
function panelMove(fx, fy) {
  if (PDRAG.on) { panelDrag(fx, fy); return true; }
  PANEL.arrowHover = panelArrowAt(fx, fy);
  const hit = panelHitAt(fx, fy);
  PANEL.hover = hit ? hit.i : -1;
  PANEL.hoverSub = hit ? hit.sub : -1;
  return !!hit || !!PANEL.arrowHover;
}
...
function panelPress(fx, fy) {
  ...
  const dir = panelArrowAt(fx, fy);
  if (dir) { SFX.page(); panelScrollBy(dir * PANEL.viewH * 0.6); return true; }
  PDRAG.on = true; PDRAG.y0 = fy; PDRAG.s0 = PANEL.target; PDRAG.moved = 0;
  PDRAG.hit = panelHitAt(fx, fy);
  return true;
}

/* the paper follows your finger, and a tap that dragged is not a tap */
function panelDrag(fx, fy) {
  if (!PDRAG.on) return false;
  const dy = fy - PDRAG.y0;
  PDRAG.moved = Math.max(PDRAG.moved, Math.abs(dy));
  if (panelScrolls() && PDRAG.moved > 2) {
    PANEL.target = Math.max(0, Math.min(panelMaxScroll(), PDRAG.s0 - dy));
...
function panelRelease(fx, fy) {
  if (!PDRAG.on) return false;
  PDRAG.on = false;
  const hit = PDRAG.hit;
  PDRAG.hit = null;
  if (PDRAG.moved > 3 || !hit) return true;
...
function bagItems() {
  ...
  const sn = Object.keys(save.snails || {}).length;
  if (sn) out.push({ t: 'row', icon: 'snail', label: 'Snail collection',
                     right: String(sn), act: () => { closePanel(); ascendToGarden(); } });
  const gl = goalList();
...
function settingsItems() {
  ...
  return [
    ...
    { t: 'row', label: 'Snail collection', icon: 'snail', right: 'GO UP',
      act: () => { closePanel(); ascendToGarden(); } },
...
function drawIcon(c, id, size) {
  ...
  switch (id) {
    ...
    case 'snail':
      pellipse(c, 7, 11, 6, 3, '#c9a06a'); P(1, 12, 4, 2, '#c9a06a');
      pcircle(c, 10, 7, 5, '#8a5f38'); pcircle(c, 10, 7, 3, '#c98a4a'); dot(c, 10, 7, '#6b4526');
      P(3, 6, 1, 3, '#c9a06a'); P(5, 5, 1, 4, '#c9a06a');
      dot(c, 3, 5, '#3a2410'); dot(c, 5, 4, '#3a2410'); break;
    case 'lot':
...
/* =========================================================================
   THE ASCENT
   The collection lives in the garden above the clouds, so you do not open it,
   you go up to it. The park drops away, every snail you have ever been sent
   rises past you, and the garden fades in out of the white.
   ========================================================================= */
function ascendToGarden(back) {
  buildGarden();
  if (!G.garden.list.length) {
    sayYou('No snails yet. They start arriving the moment you earn anything.');
    return;
  }
  ACH('ascend');
  // the ones that float past on the way up, seeded off the real collection
  G.risers = G.garden.list.slice(0, 22).map((it, i) => ({
    x: 14 + ((i * 53) % 97) / 97 * (W() - 28),
    y: H * 0.45 + ((i * 71) % 100) / 100 * H * 2.4,
    sp: 55 + ((i * 29) % 100) / 100 * 95,
    sway: (i % 7) * 0.9,
    skin: SPR.snailSkin(it.id, it.kind),
    scale: 0.75 + ((i % 4) * 0.22)
  }));
  G.backTo = back || { scene: 'game', area: G.area };
  SFX.note();
  playCine('ascend', [
    { dur: 1.5, caption: 'up', snap: true },
    { dur: 1.7, caption: '' },
    { dur: 1.3, caption: 'the garden' }
  ], () => openGarden(true));
}

function descendFromGarden() {
  const back = G.backTo || { scene: 'game', area: G.area };
  SFX.page();
  playCine('descend', [
    { dur: 1.0, caption: '' },
    { dur: 1.2, caption: '' }
  ], () => {
    G.scene = back.scene === 'heaven' ? 'heaven' : 'game';
    if (back.scene !== 'heaven') { G.area = back.area; G.areaNow = areaId(); seedPickups(); }
    G.garden.hover = -1;
    hideBubble();
    refreshActions();
  });
...
/* every riser drifts up the screen and out of the top */
function updateRisers(dt) {
  if (!G.risers) return;
  for (const r of G.risers) {
    r.y -= r.sp * dt;
    r.x += Math.sin(G.t * 1.6 + r.sway) * 8 * dt;
    if (r.y < -30) r.y = H + 20 + Math.random() * H;
  }
}

function drawRisers(c, alpha) {
  if (!G.risers) return;
  c.globalAlpha = alpha === undefined ? 1 : alpha;
  for (const r of G.risers) {
    SPR.drawSnail(c, G, { x: r.x, y: r.y, skin: r.skin, scale: r.scale, dir: 1, opened: true, noTrail: true });
  }
  c.globalAlpha = 1;
}

function openGarden(fromCine) {
  buildGarden();
...
function closeGarden() {
  if (G.backTo && G.backTo.scene !== 'heaven') { descendFromGarden(); return; }
  G.scene = 'heaven';
  G.garden.hover = -1;
  hideBubble();
  refreshActions();
...
function drawCineFrame() {
  ...
  if (name === 'death') {
    ...
    if (G.cine.i === 0) {
      // the park falls away underneath you
      const p2 = Math.pow(p, 1.5);
      dc.save();
      dc.translate(0, p2 * H * 1.1);
      drawWorld();
      dc.restore();
      dc.globalAlpha = Math.max(0, (p - 0.3) * 1.6); SPR.px(dc, 0, 0, W(), H, '#dceeff'); dc.globalAlpha = 1;
      drawRisers(dc, Math.min(1, Math.max(0, p - 0.15) * 2.4));
      SPR.drawRays(dc, G, 0.4 + p * 0.4, CX(), -10);
    } else if (G.cine.i === 1) {
      SPR.px(dc, 0, 0, W(), H, '#dceeff');
      SPR.drawCloudTunnel(dc, G, p, 1);
      drawRisers(dc, 1);
      SPR.drawRays(dc, G, 0.7, CX(), 20);
      dc.globalAlpha = Math.max(0, p - 0.72) * 3.6; SPR.px(dc, 0, 0, W(), H, '#ffffff'); dc.globalAlpha = 1;
    } else {
      SPR.drawGarden(dc, G);
      drawRisers(dc, Math.max(0, 1 - p * 1.6));
      SPR.drawRays(dc, G, 0.7 * (1 - p * 0.5), CX(), 40);
      dc.globalAlpha = Math.max(0, 1 - p * 2.1); SPR.px(dc, 0, 0, W(), H, '#ffffff'); dc.globalAlpha = 1;
    }
  } else if (name === 'descend') {
    if (G.cine.i === 0) {
      SPR.drawGarden(dc, G);
      dc.globalAlpha = Math.min(1, p * 1.6); SPR.px(dc, 0, 0, W(), H, '#ffffff'); dc.globalAlpha = 1;
    } else {
      SPR.px(dc, 0, 0, W(), H, '#e2f0ff');
      SPR.drawCloudTunnel(dc, G, p, -1);
      drawRisers(dc, Math.max(0, 1 - p));
      dc.globalAlpha = Math.max(0, p - 0.7) * 3.3; SPR.px(dc, 0, 0, W(), H, '#ffffff'); dc.globalAlpha = 1;
    }
...
function onRelease(ev) {
  if (panelOpen()) {
    const fr = ev ? toScreenPixels(ev) : { x: 0, y: 0 };
    if (panelRelease(fr.x, fr.y)) return;
  }
  grab = null;
...
cv.addEventListener('wheel', ev => {
  if (panelOpen()) {
    ev.preventDefault();
    panelScrollBy((ev.deltaY + ev.deltaX) * 0.5);
    return;
  }
  if (G.scene === 'garden') {
    ev.preventDefault();
...
function wakeHim() {
  ...
  setTimeout(() => {
    ...
    say('THE WISE OAK TREE', hadSave
      ...
      'sleepy');
    // the rules never open themselves. They are a row in the bag and in the
    // settings, and the tag top-left says what to do next anyway.
    if (!save.learned.howto) setTimeout(() => nudge('how to play is in your bag, and in the settings', 10), 5000);
...
if (/[?&]debug/.test(location.search)) {
  window.OAK = { G, save, ACH, reachEnding, startBurning, goHeaven, reincarnate,
                 ...
                 panelArrowBox, panelScrollBy, panelMaxScroll, panelRelease, PDRAG,
                 ascendToGarden, descendFromGarden, updateRisers,
